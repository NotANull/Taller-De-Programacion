...
procedure generarArbol(var a: arbol);
	...
	begin
		...
		else if(elem.numCliente = a^.dato.numCliente) then begin
			ArmarRegistroAGuardarEnLista (elem, aGuardar);
			agregarAdelante( a^.dato.pedidos, aGuardar);
		end
		else if (elem.numCliente < a^.dato.numCliente) then
...
begin
	...
	a:= nil;
	leerRegistro(pedido);
	while(pedido.numCliente <> 0) do begin
...
begin
	if (a= nil) then
		buscarPedidos:= nil
	else if(a^.dato.numCliente = numero) then
		buscarPedidos:= a^.dato.pedidos
	else if(numero < a^.dato.numCliente) then
		buscarPedidos:= buscarPedidos (a^.hi, numero)
	else
		buscarPedidos:= buscarPedidos (a^.hd, numero);
end;
...
begin
	...
	if(a = nil) then begin
		...
		if(listaPedidos = nil) then
			writeln ('El cliente con numero ', numeroClienteABuscar, ' no existe en el arbol')
		else begin
			writeln;
