...
se almacenan juntas todas sus compras (día, cantidad de productos y monto) que realizó. La lectura finaliza con cantidad de productos 0.}
	...
	begin
		...
		else if(elem.codCliente = a^.dato.codCliente) then begin
			ArmarRegistroAGuardarEnLista(elem, aGuardar);
			agregarAdelante(a^.dato.compras, aGuardar);
		end
		else if (elem.codCliente < a^.dato.codCliente) then
...
begin
	...
	a:= nil;
	leerRegistro(compra);
	while(compra.cantProductos <> 0) do begin
...
begin
	if(a = nil) then
		buscarCompras:= nil
	else if( a^.dato.codCliente = codigo) then
		buscarCompras:= a^.dato.compras
	else if(codigo < a^.dato.codCliente ) then
...
begin
	if(l= nil) then
		...
		if(l^.dato.cantProductos > maxProd) then begin
...
begin
	...
	if( a = nil) then begin
		...
		if (listaCompras = nil) then 
			writeln ('El cliente con codigo ', codigoClienteABuscar, ' no existe en el arbol')
		else begin
			writeln;
